...
def get_parser():
    ...
    parser.add_argument('--lr_schedule', help='Number of epochs for learning rate piecewise.', default=[4, 7])
    parser.add_argument('--lr_values', help='value of learning rate for each epoch piecewise.', default=[0.01, 0.003, 0.001])
    ...
    parser.add_argument('--pretrained_model', type=str, default='./output/ckpt_best/acc96.5',
                        help='Load a pretrained model before training starts.')
    parser.add_argument('--ckpt_index', default='MobileFaceNet_iter_180000.ckpt', help='ckpt file index.')
...
if __name__ == '__main__':
    with tf.Graph().as_default():
        ...
        learning_rate = tf.train.piecewise_constant(epoch, boundaries=args.lr_schedule,
                                                    values=args.lr_values, name='lr_schedule')
        ...
        path = pretrained_model + '/' + args.ckpt_index
                ...
                try:
                    ...
                    if count > 0 and count % args.validate_interval == 0:
                        ...
                        for ver_step in range(len(ver_list)):
                            ...
                            with open(os.path.join(log_dir, '{}_result.txt'.format(ver_name_list[ver_step])),
                                      'at') as f:
                                f.write('%d\t%.5f\t%.5f\t' % (count, np.mean(accuracy), val))
